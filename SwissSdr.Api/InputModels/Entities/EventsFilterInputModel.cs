...
{
	public class EventsFilterInputModel : IQueryCreator<Event_Filter.Result>, IQueryValueProvider
	{
		...
		public IRavenQueryable<Event_Filter.Result> CreateQuery(IAsyncDocumentSession session)
		{
			var query = session.Query<Event_Filter.Result, Event_Filter>();
			...
			if (Begin.HasValue)
			{
				query = query.Where(x => Begin.Value <= x.End);
			}
			...
			{
				query = query.Where(x => x.Languages.ContainsAny(Languages));
			}
